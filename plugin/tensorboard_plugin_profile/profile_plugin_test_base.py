...
try:
  ...
  from tensorboard_plugin_profile.tb_free import plugin_asset_util  # pylint: disable=g-import-not-at-top
...
def write_empty_event_file(logdir):
  ...
  with open(
      os.path.join(logdir, 'events.out.tfevents.profile-empty'), 'w+'
  ) as f:
    f.write('dummy')
...
class BaseProfilePluginTest(absltest.TestCase):
  ...
  def __init__(self, methodname):
    ...
    self._temp_dir = None
    self.multiplexer: context.EventMultiplexer = None
