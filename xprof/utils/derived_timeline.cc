...
#include "xprof/utils/host_offload_utils.h"
...
std::vector<XEventMetadata*> GetOrCreateHloOpEventsMetadata(
    tsl::profiler::XPlaneBuilder& xplane, const GpuEventStats& stats,
    const Symbol symbol) {
  ...
  for (absl::string_view hlo_op_name : stats.hlo_op_names) {
    ...
    if (!symbol.hlo_text.empty()) {
      tsl::profiler::XStatsBuilder<XEventMetadata> event_stats(
          hlo_op_event_metadata, &xplane);
...
inline int64_t GetDerivedLineId(int64_t first_derived_line_id,
  ...
  return first_derived_line_id +
         (target_line_id - tsl::profiler::kThreadIdTfNameScope);
}
...
std::string GetDerivedLineName(int64_t first_derived_line_id,
  ...
  switch (offset) {
    case kThreadIdTfNameScope - kThreadIdTfNameScope:
      return absl::StrCat(tsl::profiler::kTensorFlowNameScopeLineName, suffix);
    case tsl::profiler::kThreadIdHloOp - kThreadIdTfNameScope:
      return absl::StrCat(tsl::profiler::kXlaOpLineName, suffix);
    case tsl::profiler::kThreadIdHloModule - kThreadIdTfNameScope:
      return absl::StrCat(tsl::profiler::kXlaModuleLineName, suffix);
    case tsl::profiler::kThreadIdTfOp - kThreadIdTfNameScope:
      return absl::StrCat(tsl::profiler::kTensorFlowOpLineName, suffix);
    case tsl::profiler::kThreadIdSource - kThreadIdTfNameScope:
      return absl::StrCat(tsl::profiler::kSourceLineName, suffix);
    default:
...
std::vector<int64_t> DeriveEventsFromAnnotationsForLines(
  ...
  int64_t start_timestamp_ns =
      tsl::profiler::GetStartTimestampNs(*device_trace);
...
void DeriveStepEventsFromGroups(
  ...
  int64_t start_timestamp_ns =
      tsl::profiler::GetStartTimestampNs(*device_trace);
  DerivedXLineBuilder steps(&plane_builder, tsl::profiler::kThreadIdStepInfo,
...
void DeriveEventsFromHostTrace(
  ...
  int64_t host_plane_start = tsl::profiler::GetStartTimestampNs(*host_trace);
  for (int i = 0; i < num_devices; ++i) {
    if (per_device_launch_info[i].empty()) continue;
    int64_t device_plane_start =
        tsl::profiler::GetStartTimestampNs(*device_traces[i]);
    ...
    tsl::profiler::XLineBuilder launch_line =
        device_plane.GetOrCreateLine(tsl::profiler::kThreadIdKernelLaunch);
    launch_line.SetName(tsl::profiler::kKernelLaunchLineName);
...
void GenerateDerivedTimeLines(
  ...
  const XPlane* namespace_tree_plane = tsl::profiler::FindPlaneWithName(
...
void DeriveLinesFromStats(XPlane* device_trace) {
  ...
  DerivedXLineBuilder tf_ops(
      ...
      tsl::profiler::kTensorFlowOpLineName, start_timestamp_ns, {});
  DerivedXLineBuilder tf_name_scope(&plane_builder,
                                    tensorflow::profiler::kThreadIdTfNameScope,
                                    tsl::profiler::kTensorFlowNameScopeLineName,
                                    start_timestamp_ns, {&tf_ops});
  DerivedXLineBuilder source(
      &plane_builder, tensorflow::profiler::kThreadIdSource,
      tsl::profiler::kSourceLineName, start_timestamp_ns, {});
...
void DeriveLinesForXlaCpuOps(XPlane* host_trace) {
  if (host_trace == nullptr ||
      !absl::StartsWith(host_trace->name(),
                        tsl::profiler::kHostThreadsPlaneName))
    return;
  ...
  visitor.ForEachLine([&](const XLineVisitor& line) {
    ...
    DerivedXLineBuilder tf_ops(
        ...
        absl::StrCat(line.Name(), "-", tsl::profiler::kTensorFlowOpLineName),
        start_timestamp_ns, {});
    DerivedXLineBuilder tf_name_scope(
        &plane_builder, line_id++,
  ...
  tsl::profiler::RemoveEmptyLines(&destination_plane);
  tsl::profiler::MergePlanes(destination_plane, host_trace);
}
