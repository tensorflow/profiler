...
#ifndef XPROF_UTILS_EVENT_SPAN_H_
...
#include "xprof/protobuf/steps_db.pb.h"

namespace tsl {
namespace profiler {

using ::tensorflow::profiler::OpMetricsDb;
using ::tensorflow::profiler::AllReduceInfo;
using ::tensorflow::profiler::AllReduceDbResult;
using ::tensorflow::profiler::DeviceMemoryTransfer;
using ::tensorflow::profiler::PrecisionStats;
...
class StepDetails {
 public:
  ...
  const absl::flat_hash_map<uint32, AllReduceDbResult>&
  Collectives() const {
    return collectives_;
  }
  const std::vector<DeviceMemoryTransfer>&
  DeviceMemoryTransfers() const {
    return device_memory_transfers_;
  }

  absl::flat_hash_map<uint32, OpMetricsDb>&
  PerCoreOpMetricsDb() {
  ...
  void AddCollectiveOpEvent(uint64 core_id,
  ...
  void SetPerCoreOpMetricsDb(OpMetricsDb db,
                             uint32 core_id) {
 ...
 private:
  ...
  absl::flat_hash_map<uint32, AllReduceDbResult>
      collectives_;
  ...
  // durations.
  std::vector<DeviceMemoryTransfer>
      device_memory_transfers_;
  std::string step_name_;

  absl::flat_hash_map<uint32, OpMetricsDb>
      per_core_op_metrics_db_;
};
...
#endif  // XPROF_UTILS_EVENT_SPAN_H_
