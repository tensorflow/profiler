...
using ::tensorflow::DataTypeFromString;
using ::tensorflow::DeviceProperties;
using ::tensorflow::OpInfo;
...
static OpInfo::TensorProperties GetTensorProperties(absl::string_view info) {
  ...
  DataType data_type = tensorflow::DT_INVALID;
...
tensorflow::grappler::DeviceInfo TfOpRoofLineCostEstimator::GetDeviceInfo(
    const DeviceProperties& device) const {
  // Hypothetical devices that is used to measure peak flops and memory bytes
  // accessed.
  return tensorflow::grappler::DeviceInfo(/*gigaops=*/1, /*gb_per_sec=*/1);
}
...
TfOpRoofLineCostEstimator::OpRoofLineStats TfOpRoofLineCostEstimator::Predict(
  ...
  tensorflow::grappler::OpContext op_context;
  ...
  tensorflow::grappler::Costs costs = PredictCosts(op_context);
  ...
  return {
      /*flops=*/static_cast<tsl::uint64>(costs.compute_time.count()),
      /*bytes_accessed=*/static_cast<tsl::uint64>(costs.memory_time.count()),
